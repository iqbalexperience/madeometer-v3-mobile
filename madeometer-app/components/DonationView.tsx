import { CreditCard, ExternalLink, Heart, Loader2, MessageSquare, ShieldCheck, Trophy, User } from 'lucide-react';
...
const DonationView: React.FC<DonationViewProps> = ({ session }) => {
    ...
    const handleDonate = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/madeometer/db/supporters', {
                method: 'POST',
            ...
            if (supporter.id) {
                ...
                toast.error(t('failed_init_donation'));
            }
        } catch (err) {
        ...
        <>
            ...
            <form onSubmit={handleDonate} className="mb-8 bg-gray-50 rounded-2xl p-5 border border-gray-100">
                ...
                <button
                    ...
                    className="w-full py-3.5 rounded-xl bg-[#d35457] text-white font-bold shadow-lg shadow-[#d35457]/30 hover:bg-[#5851E3] transition-all active:scale-[0.98] text-[13px] flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed group"
                >
                    {isSubmitting ? (
                        <>{t('processing')}...</>
                    ) : (
                    ...
                    <>
                        ...
                        {donors.length < total && (
                            <button
                                ...
                                className="w-full py-3 text-xs font-bold text-gray-500 hover:text-gray-900 transition-colors flex items-center justify-center gap-2"
                            >
                                {isLoadingMore ? <Loader2 className="w-4 h-4 animate-spin" /> : t('load_more')}
                            </button>
                        )}
                    </>
                ) : (
                    <div className="py-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        <p className="text-xs text-gray-400 font-medium italic">{t('no_history_desc')}</p>
                    </div>
                )}
            </div>
