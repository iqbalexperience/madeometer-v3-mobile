...
import { Heart, Trophy, User, MessageSquare, CreditCard, ShieldCheck, ExternalLink, X } from 'lucide-react';
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
                {[5, 10, 25].map((amt) => (
                    <button 
                        ...
                        className={`py-2.5 rounded-xl border font-bold text-[13px] transition-all ${
                            selectedAmount === amt 
                ...
                <div key={donor.id} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 animate-in slide-in-from-bottom-2 duration-300 fill-mode-backwards" style={{ animationDelay: `${idx * 50}ms` }}>
                    <div className="flex items-start justify-between mb-1">
                        <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 flex items-center justify-center font-bold rounded-full text-xs shadow-sm ${
                                donor.amount >= 50 ? 'bg-amber-100 text-amber-700 border border-amber-200' : 
